...
def calMultiF1Score(true, pred) -> dict:
    return classification_report(true, pred, zero_division=0, output_dict=True)
...
def train(args):
    ...
    train_dataset = CustomDataset(args.train_path, train_df.path.values, train_df.mask_class.values, transforms=transform)
    ...
    val_dataset = CustomDataset(args.train_path, val_df.path.values, val_df.mask_class.values, transforms=transform)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        with torch.no_grad():
            ...
            for val_data in val_loader:
                ...
                outs = model(inputs)
                preds = torch.argmax(outs, dim=-1)
                f1_score = calMultiF1Score(preds.detach().cpu(), labels.detach().cpu())
                ...
                f1_score_items.append(f1_score['weighted avg']['f1-score'])
                
            if sum(f1_score_items) / len(f1_score_items) > best_f1_score:
                best_f1_score = sum(f1_score_items) / len(f1_score_items)
        ...
        print(f'EPOCH: {epoch}/{args.epochs} | train loss: {train_loss:.3f}, train acc: {train_acc:.3f} | val loss: {val_loss:.3f}, val acc: {val_acc:.3f}, f1 score: {best_f1_score}')
